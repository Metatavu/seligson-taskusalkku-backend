...
def get_token_bearerAuth(credentials: HTTPAuthorizationCredentials = Depends(bearer_auth)) -> TokenModel:
    ...
    if not oidc_auth_server_url:
      raise HTTPException(status_code=500, detail="Missing OIDC_AUTH_SERVER_URL env variale")
      
    oidc_audience = os.environ["OIDC_AUDIENCE"]
    if not oidc_audience:
      raise HTTPException(status_code=500, detail="Missing OIDC_AUDIENCE env variale")
