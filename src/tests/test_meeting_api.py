...
class TestMeeting:
    ...
    def test_create_meeting(self, client: TestClient, user_1_auth: BearerAuth, smtp: SmtpContainer):
        response = client.post(f"/v1/meetings", auth=user_1_auth, data=json.dumps(example_meeting))
        assert response.status_code == 200
        assert example_meeting == response.json()
    ...
    def test_create_meeting_anonymous(self, client: TestClient, backend_mysql: MySqlContainer,
        ...
        response = client.post(f"/v1/meetings", auth=anonymous_auth, data=json.dumps(example_meeting))
        assert response.status_code == 200
        assert example_meeting == response.json()
